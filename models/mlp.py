...
class MLPdecoder(nn.Module):
    ...
    """
    def __init__(self, n_layers, hidden_dim, norm,
                 learn_norm=True, track_norm=False, *args, **kwargs):
        super(MLPdecoder, self).__init__()

        self.init_embed = nn.Linear(4,hidden_dim, bias=True)
    ...
    def forward(self, x, batch, graph=None):
        """
        Args:
        ...
        """
        # x = self.init_embed(x)
        x = global_add_pool(x, batch) # try mean pooling
